...
export class App {
  ...
  private filterNewsPanelsByRegion(region: import('@/config/geographic-regions').GeographicRegion): void {
    ...
    if (region.id !== 'global' && allowedSources.size > 0) {
      // Filter the main news array
      const filteredNews = this.allNews.filter(item => {
        // Always show global sources (Reuters, Bloomberg, UN, etc.)
        const isGlobalSource = allowedSources.has(item.source) ||
          item.source.includes('Reuters') ||
          item.source.includes('Bloomberg') ||
          item.source.includes('UN News');
        return isGlobalSource || allowedSources.has(item.source);
      });

      // Update each news panel with filtered items
      Object.values(this.newsPanels).forEach(panel => {
        const categoryKey = (panel as any).category;
        if (categoryKey && this.newsByCategory[categoryKey]) {
          const categoryFiltered = this.newsByCategory[categoryKey].filter(item => {
            const isGlobalSource = allowedSources.has(item.source) ||
              item.source.includes('Reuters') ||
              item.source.includes('Bloomberg') ||
              item.source.includes('UN News');
          ...
          panel.renderNews(categoryFiltered);
        }
      });

      console.log(`[App] Filtered news: ${filteredNews.length}/${this.allNews.length} items, ${allowedSources.size} sources for ${region.label}`);
    } else {
      // Global view - show all news
      Object.values(this.newsPanels).forEach(panel => {
        const categoryKey = (panel as any).category;
        if (categoryKey && this.newsByCategory[categoryKey]) {
          panel.renderNews(this.newsByCategory[categoryKey]);
        }
      });
      console.log(`[App] Showing all news (global view)`);
    }
  }
